 ...
 * a roof above. Ported from the Vape client's module of the same design, which ships under the
 * name "Block-In" - the class and its helpers are called Clutch there too.
 * <p>
 ...
 *     <li>The aim controller's acceleration curve is not in the recovered files, so aiming turns at
 *     a constant rate - the configured aim speed - instead.</li>
 * </ul>
 */
public class Clutch extends Module {
    ...
    public Clutch() {
        super("Clutch", false, false, "Blocks you in with walls and a roof");
    }

    // ---------------------------------------------------------------- lifecycle

    /** True while Clutch owns the rotation, so the aim assist knows to stay out of the way. */
    ...
    private void stop(String reason) {
        if (reason != null && Myau.notificationManager != null) {
            Myau.notificationManager.add("Clutch: " + reason, 0xFFAA00);
        }
    ...
    public static void turn(float partialTicks) {
        ...
        Module module = Myau.moduleManager.modules.get(Clutch.class);
        if (module instanceof Clutch && module.isEnabled()) {
            ((Clutch) module).applyTurn();
        }
    }
